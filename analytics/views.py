...
def order_insights(request):
    ...
    template = 'partials/analytics/order_insights.html' if is_htmx else 'analytics/order_insights.html'
...
def ledger(request):
    ...
    if is_htmx and hx_target == 'ledger-results':
        template = 'partials/analytics/ledger_results.html'
    elif is_htmx:
        template = 'partials/analytics/ledger.html'
    else:
        template = 'analytics/ledger.html'
...
def email_broadcast(request):
    ...
    template = 'partials/analytics/email_broadcast.html' if is_htmx else 'analytics/email_broadcast.html'
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            toast = {'message': f'Successfully sent to {len(email_list)} users', 'type': 'success'}
            if not is_htmx:
                messages.success(request, toast['message'])
        except Exception as e:
            toast = {'message': f'Error sending mail: {str(e)}', 'type': 'error'}
            if not is_htmx:
                messages.error(request, toast['message'])
